...
import type { Itinerary, GeoPoint } from "@/shared/types";
...
export function RouteMap({ it, start, end }: { it: Itinerary; start: GeoPoint; end: GeoPoint }) {
  ...
  useEffect(() => {
    ...
    stops.forEach((s, i) => pin(s.d.lat, s.d.lng, '<span class="rmk">' + (i + 1) + "</span>", 28, nm(s.d.name)));
    ...
    roadGeometry(via, ac.signal).then((geo) => {
      if (geo.length <= via.length) return; // fallback came back — keep the dashes
  ...
  return (
    <div className="routemap">
      <div className="mapwrap" style={{ margin: 0, height: 300 }}>
